...
RUN echo '#!/bin/sh' > /app/start.sh && \
    ...
    echo '    echo "Database found, checking schema..."' >> /app/start.sh && \
    echo '    sqlite3 /app/wedding_rsvp.db "ALTER TABLE weddings ADD COLUMN is_active BOOLEAN DEFAULT 1;" 2>/dev/null || echo "Column is_active already exists"' >> /app/start.sh && \
    echo '    sqlite3 /app/wedding_rsvp.db "ALTER TABLE weddings ADD COLUMN color_scheme TEXT DEFAULT \"marsala\";" 2>/dev/null || echo "Column color_scheme already exists"' >> /app/start.sh && \
    echo '    sqlite3 /app/wedding_rsvp.db "ALTER TABLE weddings ADD COLUMN background_color TEXT DEFAULT \"#9c2851\";" 2>/dev/null || echo "Column background_color already exists"' >> /app/start.sh && \
    echo '    sqlite3 /app/wedding_rsvp.db "ALTER TABLE weddings ADD COLUMN text_color TEXT DEFAULT \"#ffffff\";" 2>/dev/null || echo "Column text_color already exists"' >> /app/start.sh && \
